...
namespace ECS.Core
{
    ...
    public class Circuit
    {
        ...
        public Circuit(Node head, int nodeCount, int srcCount)
        {
            ...
            if (nodeCount < 1) throw new ArgumentOutOfRangeException(nameof(nodeCount), nodeCount, "Number of nodes must be greater than zero");
            if (srcCount < 1) throw new ArgumentOutOfRangeException(nameof(srcCount), srcCount, "Number of voltage sources must be greater than zero");
            Head = head;
            NodeCount = nodeCount;
